...
public class AmrIngestionRepositoryImpl implements AmrIngestionRepository {
    ...
    @Override
    public String findDtuongQlyByAccountId(String maKhang) {
        try {
    ...
    @Override
    public boolean tryAcquireBillingTriggerGate(String dtuongQly, String maKhang, String month, int period) {
        try {
    ...
    @Override
    public void logIncompleteStatus(String maKhang, String month, String dtuongQly, int period, String missingStr) {
        ...
        jdbcTemplate.update(updateSql, maKhang, month, dtuongQly, period, missingStr);
    }
    ...
    public List<Map<String, Object>> getKh2tpPmaxStatus(String maKhang, String month, int period) {
        ...
        if (!stackingRelation.isEmpty()) {
            Integer pmaxCount = jdbcTemplate.queryForObject(
                ...
                Integer.class, maKhang, month, period
            );
    ...
    @Override
    public List<Map<String, Object>> getReactivePowerStatus(String maKhang, String month, int period) {
        ...
        return jdbcTemplate.queryForList(sql, maKhang, month, period);
    }
    ...
    @Override
    public void updateCustomerBillingStatus(String maKhang, String month, int period, String status, String errorMsg) {
        String sql = "INSERT INTO trang_thai_tinh_toan_kh " +
                "(ma_khang, thang_chu_ky, dtuong_qly, ky_chot, trang_thai, thong_bao_loi, updated_at) " +
                "VALUES (?, ?, ?, ?, ?, ?, NOW()) " +
                "ON CONFLICT (ma_khang, thang_chu_ky, ky_chot) DO UPDATE SET " +
                "trang_thai = EXCLUDED.trang_thai, thong_bao_loi = EXCLUDED.thong_bao_loi, updated_at = NOW()";
        String dtuongQly = findDtuongQlyByAccountId(maKhang);
        if (dtuongQly == null) dtuongQly = "UNKNOWN";
        
        jdbcTemplate.update(sql, maKhang, month, dtuongQly, period, status, errorMsg);
    }
    ...
    @Override
    public void logIngestionLifecycle(String maKhang, String meterPointId, String month, Integer period, String step, String status, String detailJson, String source) {
        try {
            String sql = "INSERT INTO nhat_ky_chi_so (ma_khang, ma_ddo, thang_chu_ky, ky_chot, buoc_xu_ly, trang_thai, chi_tiet, nguon_ghi, created_at) " +
                         "VALUES (?, ?, ?, ?, ?, ?, ?::jsonb, ?, NOW())";
            jdbcTemplate.update(sql, maKhang, meterPointId, month, period, step, status, detailJson, source);
        } catch (Exception e) {
            log.error("Failed to write ingestion lifecycle log for Account: {}, error: {}", maKhang, e.getMessage());
        }
    }
}
